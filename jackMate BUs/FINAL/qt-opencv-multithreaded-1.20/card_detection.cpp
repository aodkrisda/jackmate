...
//#define _card_detection_LETTER_PRECENT 0.5 //0.16 normal cars, 0.56 big cards:::: this is a value to determin how big the letter precentage of a card is. 0.16 for normal cards
...
#define _card_detection_BORDER_FACTOR 0.5 //0.05 for big, 
            ...
            //int l = 1; //testing
        {
            ...
            for( size_t i = 0; i < contours.size(); i++ )
            {
                        ...
                        isContourConvex(Mat(approx)) )
                {
                    ...
                    if( maxCosine < 0.06 ) //0.3  //sunday - 0.05
...
void getTableFromMat(gameTable * Table, Mat* capimg){
    ...
    sortSquares(squares);// is it working ?? need & ?
  ...
  //newer code - no border - bad with turns
  vector<Point> letter_rect_temp = letter_rect;
  letter_rect_temp[0].x = letter_rect[0].x + ( (letter_rect[1].x - letter_rect[0].x)*_card_detection_LETTER_PRECENT*_card_detection_BORDER_FACTOR );
  letter_rect_temp[0].y = letter_rect[0].y + ( (letter_rect[2].y - letter_rect[0].y)*_card_detection_LETTER_PRECENT*_card_detection_BORDER_FACTOR);

  letter_rect_temp[1].x = letter_rect[0].x + ( (letter_rect[1].x - letter_rect[0].x)*_card_detection_LETTER_PRECENT + (letter_rect_temp[0].x-letter_rect[0].x));
  letter_rect_temp[1].y = letter_rect[0].y + ( (letter_rect[1].y - letter_rect[0].y)*_card_detection_LETTER_PRECENT + (letter_rect_temp[0].y-letter_rect[0].y));

  letter_rect_temp[2].x = letter_rect[0].x + ( (letter_rect[2].x - letter_rect[0].x)*_card_detection_LETTER_PRECENT + (letter_rect_temp[0].x-letter_rect[0].x));
  letter_rect_temp[2].y = letter_rect[0].y + ( (letter_rect[2].y - letter_rect[0].y)*_card_detection_LETTER_PRECENT + (letter_rect_temp[0].y-letter_rect[0].y));

  letter_rect_temp[3].x = letter_rect[0].x + ( (letter_rect[3].x - letter_rect[0].x)*_card_detection_LETTER_PRECENT + (letter_rect_temp[0].x-letter_rect[0].x));
  letter_rect_temp[3].y = letter_rect[0].y + ( (letter_rect[3].y - letter_rect[0].y)*_card_detection_LETTER_PRECENT + (letter_rect_temp[0].y-letter_rect[0].y));
  ...
  vector<Point> letter_rect_temp = letter_rect;
  //point 0
  if (letter_rect[1].x - letter_rect[0].x >0)
   letter_rect_temp[0].x = letter_rect[0].x + ( (letter_rect[1].x - letter_rect[0].x)*_card_detection_LETTER_PRECENT*_card_detection_BORDER_FACTOR );
  else
   letter_rect_temp[0].x = letter_rect[0].x + (-1)*( (letter_rect[1].x - letter_rect[0].x)*_card_detection_LETTER_PRECENT*_card_detection_BORDER_FACTOR );

  if (letter_rect[2].y - letter_rect[0].y >0)
   letter_rect_temp[0].y = letter_rect[0].y + ( (letter_rect[2].y - letter_rect[0].y)*_card_detection_LETTER_PRECENT*_card_detection_BORDER_FACTOR);
  else
   letter_rect_temp[0].y = letter_rect[0].y + (-1)*( (letter_rect[2].y - letter_rect[0].y)*_card_detection_LETTER_PRECENT*_card_detection_BORDER_FACTOR);

  //poin 1
  if (letter_rect_temp[0].x-letter_rect[0].x >0)
   letter_rect_temp[1].x = letter_rect[0].x + ( (letter_rect[1].x - letter_rect[0].x)*_card_detection_LETTER_PRECENT + (letter_rect_temp[0].x-letter_rect[0].x));
  else
   letter_rect_temp[1].x = letter_rect[0].x +( (letter_rect[1].x - letter_rect[0].x)*_card_detection_LETTER_PRECENT + (-1)*(letter_rect_temp[0].x-letter_rect[0].x));

  if (letter_rect_temp[0].y-letter_rect[0].y >0)
   letter_rect_temp[1].y = letter_rect[0].y + ( (letter_rect[1].y - letter_rect[0].y)*_card_detection_LETTER_PRECENT + (letter_rect_temp[0].y-letter_rect[0].y));
  else
   letter_rect_temp[1].y = letter_rect[0].y + ( (letter_rect[1].y - letter_rect[0].y)*_card_detection_LETTER_PRECENT + (-1)*(letter_rect_temp[0].y-letter_rect[0].y));

  //point 2
  if(letter_rect_temp[0].x-letter_rect[0].x >0)
   letter_rect_temp[2].x = letter_rect[0].x + ( (letter_rect[2].x - letter_rect[0].x)*_card_detection_LETTER_PRECENT + (letter_rect_temp[0].x-letter_rect[0].x));
  else
   letter_rect_temp[2].x = letter_rect[0].x + ( (letter_rect[2].x - letter_rect[0].x)*_card_detection_LETTER_PRECENT + (-1)*(letter_rect_temp[0].x-letter_rect[0].x));
  ...
  if(letter_rect_temp[0].x-letter_rect[0].x >0)
   ...
   letter_rect_temp[3].x = letter_rect[0].x + ( (letter_rect[3].x - letter_rect[0].x)*_card_detection_LETTER_PRECENT + (-1)*(letter_rect_temp[0].x-letter_rect[0].x));

  if (letter_rect_temp[0].y-letter_rect[0].y >0)
   letter_rect_temp[3].y = letter_rect[0].y + ( (letter_rect[3].y - letter_rect[0].y)*_card_detection_LETTER_PRECENT + (letter_rect_temp[0].y-letter_rect[0].y));
  else
   letter_rect_temp[3].y = letter_rect[0].y + ( (letter_rect[3].y - letter_rect[0].y)*_card_detection_LETTER_PRECENT + (-1)*(letter_rect_temp[0].y-letter_rect[0].y));
   */
        ...
        Point offset;
        /*double x = (middle.x - letter_rect[0].x);
  x = x*x;
  double y = (middle.y - letter_rect[0].y);
  y = y*y;
  double d = sqrt(x + y);
  d = d *_card_detection_BORDER_FACTOR*_card_detection_LETTER_PRECENT;
  */
	//	offset.x = (9*letter_rect[0].x + middle.x)/10 - letter_rect[0].x;
	//	offset.y = (9*letter_rect[0].y + middle.y)/10 - letter_rect[0].y; 
        ...
        new_card.letter = letter_rect_temp; //used to be without "temp"
        new_card.color = 0;
...
void drawSquares_cards( Mat *image, const vector<card> cards,int color )
{
    for( size_t i = 0; i < cards.size(); i++ )
    {
        ...
        text[1] = '\0';
        //				CvFont font = initfont(CV_FONT_HERSHEY_SIMPLEX, 1, 1, 0, 1, 8);
        //				putText(newImg, text, Table.players[i].cards[j].center,, cvScalar(0,255,255));
...
char send_to_recognition(card curCard,Mat image,IplImage * Glyphs[13], IpVec keypoints[13]){
    ...
    /*				Mat blurmask;
    GaussianBlur(newImg, blurmask, Size(0,0),1);
    addWeighted(newImg, 0.8, blurmask, 0.2, 0, newImg);
*/
    char c = extractvalue(newImg,Glyphs,keypoints);
    if (c== 'N'){
        //this should be replaced with something smarter... //DEBUG !!
        printf("\n did not recognized a the card letter !");
    }
    ...
    //				CvFont font = initfont(CV_FONT_HERSHEY_SIMPLEX, 1, 1, 0, 1, 8);
    //				putText(newImg, text, Table.players[i].cards[j].center,, cvScalar(0,255,255));
    putText(image, text, curCard.center,FONT_HERSHEY_SIMPLEX,1,cvScalar(0,255,255),2,8,false);
    //imshow("table - read", image);
    return c;
}
...
int is_frame_empty(Mat matimg,float level){
    ...
    printf("is_empty: %f",max_value);fflush(stdout);

    if (max_value > level) // this needs calibration
        return 1;
    else
        return 0;
...
int is_dealer_in_same_location(gameTable * curTab, gameTable* prevTab,int hand_diff){ //diff is tolerance for location change in pixels
    ...
    // if the hands are in the same location
    if ( (abs(curTab->players[0].center.x - prevTab->players[0].center.x) < hand_diff) && (abs(curTab->players[0].center.y - prevTab->players[0].center.y) < hand_diff) )
        return 1;
    else
        return 0;
...
//float is_frame_empty2(Mat mat, float level){
...
//    /* Convert the image to gray */
//    //        CvArr a(mtx);
//    //            cvCvtColor(a,matimg,CV_RGB2GRAY);

//    for (int i=0; i<mat.rows;i++){
//        for(int j=0;j<mat.cols; j++){
//            Vec3f v3f = mat.at<Vec3f>(i,j);
//            V1 = V1+ v3f[0];
//            V2 = V2+ v3f[1];
//            V3 = V3+ v3f[2];
//        }
